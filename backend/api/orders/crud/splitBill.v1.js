...
/**
 * @desc    Split an order into multiple sub-orders (split bill)
...
module.exports = async (req, res) => {
  try {
    ...
    if (!splits || splits.length < 2) {
      return res.status(400).json({ success: false, message: "Must provide at least 2 splits" });
    }
    ...
    if (original.status === "voided") {
      return res.status(400).json({ success: false, message: "Cannot split a voided order" });
    }
    ...
    for (let i = 0; i < splits.length; i++) {
      ...
      for (const si of split.items) {
        ...
        if (!origItem) {
          return res.status(404).json({ success: false, message: `Order item ${si.order_item_id} not found` });
        }
      ...
      if (childErr) {
        return res.status(500).json({ success: false, message: childErr.message });
      }
